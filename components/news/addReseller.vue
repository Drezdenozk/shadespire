            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    ...
                    <div class="form-group">
                        ...
                        <tiny-mce id="short_desc" v-model="shopModel.news.teaser" :toolbar="toolbar"></tiny-mce>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Полный текст новости:</label><br/>
                        <tiny-mce id="full_desc" v-model="shopModel.news.fulltext" :toolbar="toolbar"></tiny-mce>
                    </div>
                </div>
            </div>
            <div class="panel">
                ...
                <div class="panel-body">
                    <table class="table table-striped table-overflow dataTable">
                        ...
                        <tr>
                            <th>Превью</th>
                            <th>Ссылка</th>
                            <th width="50"><input type="file" accept="image/*" v-on:change="uploadAvatar($event, true)"></th>
                        </tr>
                        </thead>
                        <tbody>
                            <template v-if="!shopModel.aImages.length > 0">
                                <tr>
                                    <td colspan="3">
                                        Вы еще не загрузили ни одной дополнительной фотографии
                                    </td>
                                </tr>
                            </template>
                            <template v-else>
                                <tr v-for="aImage in shopModel.aImages">
                                    <td width="100"><img :src="aImage" width="100" /></td>
                                    <td colspan="2"><input type="text" class="form-control" :value="aImage" readonly/></td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
            ...
            if (response.data.status === 'success') {
              if (response.data.message === 'news_created_successfully') {
                ...
                if (response.data.id !== undefined) {
                  self.$router.replace('/panel/news/edit/' + response.data.id)
                } else {
                  self.$router.replace('/panel/news')
                }
